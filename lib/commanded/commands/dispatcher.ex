defmodule Commanded.Commands.Dispatcher do
  ...
  def dispatch(%Payload{} = payload) do
    ...
    Logger.warn(fn -> "Going to dispatch #{inspect payload.command.__struct__}" end)
  ...
  do
    ...
    {_, _, [event]} = result
    Logger.warn(fn -> "DISPATCHER:=> [[[#{aggregate_module}]]] just executed #{inspect event.__struct__}" end)
    case result do
